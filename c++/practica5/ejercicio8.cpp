...
template <typename T> void push(Nodo* &pila, T valor)
{
	Nodo* nuevo = new Nodo;
	nuevo->dato = valor;
	nuevo->sig = pila;
...
template <typename T> void agregar(Nodo*& cola, T valor)
{
	Nodo* nuevo = new Nodo;
	nuevo->dato = valor;
	nuevo->sig = nullptr;
...
template <typename T> void mostrar(Nodo* lista)
{
	while (lista != nullptr) {
...
template <typename T> bool buscar_lineal(T clave, Nodo* lista, int (*criterio)(T, T))
{
	while (lista != nullptr) {
...
int main()
{
  ...
  Nodo* final2=mayorpuntero(pila);
  cout<<"El puntero a el struct con el numero mas grande de la lista es "<<final2<<"con el numero"<<final2->dato;
