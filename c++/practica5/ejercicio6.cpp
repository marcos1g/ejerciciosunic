...
struct Nodo
{
	char dato; // valor que contiene el nodo
...
bool esCapicuaA(int total, string palabra, Nodo *&pila)
{
	Nodo *aux = nullptr;
	int final;
	...
	{
		push(pila, palabra[i]);
		agregar(aux, palabra[i]);
	}
	for (final = 0; final < total; final++)
	{
		if (pop(aux) != pop(pila))
		{
			return false;
...
bool esCapicuaB(int total, string palabra, Nodo *&pila)
{
	int e = 0;
	for (int i = 0; i < total; i++)
	{
	...
	for (e = 0; pila != nullptr && palabra[e] == pop(pila); ++e)
	{
	}
	return e == total ? true : false;
}
int main()
{
	string palabra;
	...
	bool capicua = esCapicuaB(tamaño, palabra, pila);
